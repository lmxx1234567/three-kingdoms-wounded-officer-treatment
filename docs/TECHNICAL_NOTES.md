...
Eight dilemma records provide the static UI layouts required by the game: the original treatment layouts for both, either, or neither unique reward; three affordability layouts that omit treatment and/or recruitment; and a generic renowned physician. Before choosing a variant, Lua checks the treasury, scans every faction inventory and every spawned character for the vanilla Hua Tuo follower and manual CEOs, and suppresses the event when no displayed action can succeed. `can_create_ceo` is also checked before display and again before resolution.

`DilemmaChoiceMadeEvent` maps the chosen dilemma and zero-based choice index to treatment, recruitment, confiscation, or decline. Treatment revalidates the patient and treasury, replaces the wound, and charges 2000 only after replacement succeeds. Recruitment rechecks uniqueness, charges 1000, adds the vanilla follower to the faction CEO inventory, and concludes that faction's Hua Tuo story. Confiscation adds the vanilla manual, applies the custom five-turn `wota_loyalty_effect_hua_tuo_confiscated` to every current faction character, and also concludes the story. The negative satisfaction factor is displayed as the reason “Confiscated Hua Tuo's Estate”; it has no positive counterpart or separate physician-targeting mechanic. Future eligible encounters for a concluded faction use the generic two-choice physician dilemma when the faction can afford treatment.
